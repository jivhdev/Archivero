...
Ver `preguntas/Caso-6.md` para el texto completo. Los 3 puntos son independientes entre sí.

### Punto 1 — "Pendientes de distribuir" no se actualizaba en vivo
Causa real (no era el problema de notificación/refresco que se sospechaba en Caso-6.md — ambas listas de `MainWindow` ya comparten el mismo `CargarPendientes()`, llamado por los mismos eventos): el evento `Created` del `FileSystemWatcher` dispara apenas Windows crea el archivo destino, no cuando termina de copiarse. Con archivos grandes (una guía escaneada en imagen pesa mucho más que un PDF con texto) es común que el proceso que copia todavía tenga el archivo bloqueado en ese momento; `LectorPdf` tiraba una excepción que el `catch` genérico de `VigilanciaCarpetaService.ProcesarArchivo` se tragaba en silencio (RNF-2), y el archivo nunca llegaba a pendientes hasta el próximo reinicio (que sí funcionaba, porque para entonces la copia ya había terminado — de ahí que pareciera "solo se actualiza al reabrir").

Se agregó una espera activa (hasta 3 segundos, reintentando cada 300ms) que intenta abrir el archivo de solo lectura antes de tocarlo. Además, botón "Recargar" en "Pendientes de distribuir" para forzar el refresco a mano, sin importar el resultado del fix de fondo (pedido explícitamente en Caso-6.md sin condicionarlo al punto 1). PR: [#24](https://github.com/jivhdev/Archivero/pull/24).

### Punto 2 — "Guardados recientes" persistidos
El historial de "Guardados automáticamente" vivía solo en memoria de `MainWindow`: sin límite de tamaño dentro de una misma sesión, pero se perdía por completo cada vez que Archivero se cerraba de verdad (no solo minimizado a la bandeja) y se volvía a abrir — de ahí que pareciera "olvidarse casi enseguida". Nueva tabla `GuardadosRecientes` + `GuardadoRecienteRepository`: guarda cada guardado automático en la base, recortado a los últimos 20 (piso de 10 pedido por Javier, con margen). PR: [#25](https://github.com/jivhdev/Archivero/pull/25).
